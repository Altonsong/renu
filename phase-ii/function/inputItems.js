...
addons.forEach((addon) => {
  addonsHTML += `
    ...
    <div class="addonSerial"">${addon.serial}</div>
      <div class="addonName"">${addon.name}</div>
      <div class="addonsDetail"> 
        <img src="${addon.image}" class="imgItems">
        <div>${addon.detail}</div>
      </div>
      <div class="addonPrice">${addon.price}</div>
      <input class="addonsCheckbox" type="checkbox" ${addon.isChecked ? 'checked' : ''}>    </div>
  `;
})

document.querySelector('.js-addons-area').innerHTML = addonsHTML;
