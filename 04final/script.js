...
function updateSubTotalCost() {
    ...
    if (subTotalCost === 0) {
        ...
        if (testGProfit > 18000) {
            stdPrice = totalCost + 18000 + addonTotal;
        } else if (testGProfit <= 2000) {
            stdPrice = totalCost + 2000 + addonTotal;
        } else {
            stdPrice = totalCost + testGProfit + addonTotal;
        }
    }
...
document.addEventListener('DOMContentLoaded', function () {
  const checkboxes = document.querySelectorAll('.addonsCheckbox');

  checkboxes.forEach((checkbox, index) => {
    checkbox.addEventListener('change', function () {
      const addon = addons[index];
      const addonPrice = parseInt(addon.price);

      if (this.checked) {
        window[addon.serial] = addonPrice;
        console.log(`${addon.serial} is checked. Value: ${window[addon.serial]}`);
      } else {
        delete window[addon.serial];
        console.log(`${addon.serial} is unchecked.`);
      }
  ...
  function updateAddonTotal() {
    ...
    // Add the price of each selected addon to addonTotal
    addons.forEach(addon => {
      if (window[addon.serial]) {
        addonTotal += window[addon.serial];
      }
    });

    console.log(`Updated addonTotal: ${addonTotal}`);
  }

//   function updateSubTotalCost() {
//     // Assuming the existing variables like locationCost, kitchenCost, etc. exist
//     let subTotalCost = locationCost + kitchenCost + bath1Cost + bath2Cost + bath3Cost + floorCost + paintCost + totalDemoCost + totalDeliCost + totalTravCost + totalClenCost + addonTotal;

//     console.log(`Updated subTotalCost: ${subTotalCost}`);
//   }
});
